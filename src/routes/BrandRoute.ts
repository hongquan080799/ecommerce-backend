...
import * as service from '../service/BrandService'
...
router.post('/', async (req, res) => {
    try {
        const category = await service.saveBrand(req.body)
...
router.put('/', async (req, res) => {
    try {
        const category = await service.updateBrand(req.body)
        res.status(200).json(category)
    } catch(err: any) {
        res.status(500).send({
            message: err.message,
...
router.delete('/:id', async (req, res) => {
    try {
        const category = await service.deleteBrand(Number(req.params.id))
        res.status(200).json(category)
    } catch(err: any) {
        res.status(500).send({
            message: err.message,
