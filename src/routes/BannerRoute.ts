...
import * as service from '../service/BannerService'
...
router.post('/', async (req, res) => {
    try {
        const category = await service.saveBanner(req.body)
...
router.put('/', async (req, res) => {
    try {
        const category = await service.updateBanner(req.body)
        res.status(200).json(category)
    } catch(err: any) {
        res.status(500).send({
            message: err.message,
...
router.delete('/:id', async (req, res) => {
    try {
        const category = await service.deleteBanner(Number(req.params.id))
        res.status(200).json(category)
    } catch(err: any) {
        res.status(500).send({
            message: err.message,
